...
vector<Route> vecinoMasCercano(const VRPLIBReader& instancia) {
    ...
    vector<vector<double>> distancias_ordenadas = {};
    vector<vector<double>> distancias_copia = {};

    vector<vector<int>> calcularVecinosMasCercanos(const vector<vector<double>>& distancias){
        int n = distancias.size();
        vector<vector<int>> vecinosMasCercanos(n);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            sort(distanciasDesdeI.begin(), distanciasDesdeI.end());

            // guardar solo los índices de los vecinos
            for (const auto& par : distanciasDesdeI) {
                vecinosMasCercanos[i].push_back(par.second);
            }   
    }
